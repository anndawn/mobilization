queue() 
	.defer(d3.csv, "data/values.csv")
	.defer(d3.csv, "data/ID of parent.csv")
	.defer(d3.json, "data/newmy.json")
	.await(drawAll);
...
function drawAll(error, ageCSV, idCSV, occupations) {
	...
	var padding = 20,
		...
		height = (mobileSize | window.innerWidth < 768 ? width : window.innerHeight - 30);
console.log($("#chart").innerWidth());
	...
	var colorCircle = d3.scale.linear()
			.domain([-1, 5])
			.range(["hsl(165,80%,80%)", "hsl(210,30%,40%)"])
			.interpolate(d3.interpolateHcl);
			
	var colorBar = d3.scale.ordinal()
		.domain(["Benin", "Nigeria", "Liberia",  "Guinea", "Sierra Leone", "Togo","Regional"])
		.range(["#e6da6f", "#f89a01", "#aeb7b2", "#c49b88", "#ef8a82", "#b781c3", "#b2c78b"]);
			...
			if(node.ID in dataById) {
				...
				if(node.ID.lastIndexOf(currentID, 0) === 0  & !hidden) {
					...
					if(drawTitle & showText) {	
						...
						chosenContext.fillText("Total "+commaFormat(node.size)+" (in dollars)", nodeX, nodeY + -0.75 * nodeR);
					...
					if (Math.round(barHeight) > 1) {
						...
						for (var j = 0; j < bars.length; j++) {
							...
							bar.barPiecePosition = nodeY + barChartHeightOffset*4*nodeR + j*eachBarHeight - barChartHeight*nodeR;
		...
		for (var i = 0; i < nodeCount; i++) {
			...
			if(typeof node.parent !== "undefined" & typeof node.children !== "undefined") {
				if(node.name !== "occupation" & !hidden & showText & $.inArray(node.name, kids) >= 0) {
					...
					var fontSizeTitle = Math.round(nodeR / 7);
					if (fontSizeTitle<15) {
						fontSizeTitle=15
					}
					console.log(node);
				...
				if (node.name == "Education" && fontSizeTitle > 4) {
					drawCircularText(chosenContext, node.name.replace(/,? and /g, ' & '), fontSizeTitle, titleFont, nodeX, nodeY, nodeR,-30, 0);
				} else if (node.name == "Communication and Information" && fontSizeTitle > 4) {
					drawCircularText(chosenContext, node.name.replace(/,? and /g, ' & '), fontSizeTitle, titleFont, nodeX, nodeY, nodeR, 40, 0);
				} else if (node.name == "ER5 strengthened management of natural resources" && fontSizeTitle > 4) {
					drawCircularText(chosenContext, node.name.replace(/,? and /g, ' & '), fontSizeTitle, titleFont, nodeX, nodeY, nodeR, 40, 0);
				} 
				else if (fontSizeTitle > 4) drawCircularText(chosenContext, node.name.replace(/,? and /g, ' & '), fontSizeTitle, titleFont, nodeX, nodeY, nodeR, rotationText[counter], 0);
				}//if
				counter = counter + 1;
			}//if
